...
enum RouteChecklistType { route, vehicle }

extension RouteChecklistTypeExt on RouteChecklistType {
  int get value => index;

  static RouteChecklistType fromInt(int value) {
    return RouteChecklistType.values[value];
  }
}
...
class RouteChecklist implements BaseModel {
  static final String sqlTable = '''
    CREATE TABLE IF NOT EXISTS route_checklist (
      ...
      type INTEGER NOT NULL -- 0 = Route 1 = Vehicle
    );
  ''';
  ...
  factory RouteChecklist.buildFromMap(Map<String, dynamic> map) {
    return RouteChecklist(
      ...
      type: RouteChecklistTypeExt.fromInt(map['type'] ?? 0),
    );
  }
  ...
  static Future<int> makeNew(String name, RouteChecklistType type) async {
    final db = await AppDatabase.instance.db;
